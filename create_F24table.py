...
def create_table():
    ...
    # 计算上周一和本周一的日期
    last_monday = everyday_performance['日期'].max() - pd.DateOffset(days=everyday_performance['日期'].max().weekday() + 7)
    ...
    yesterday_clockin_data = yesterday_clockin_data.rename(columns={'线下办公人数': '昨日线下打卡人数', '线上办公人数': '昨日线上打卡人数'})
    yesterday_clockin_data = yesterday_clockin_data[['小组长','昨日线下打卡人数','昨日线上打卡人数']]
    yesterday_clockin_data['昨日打卡总人数'] = yesterday_clockin_data['昨日线下打卡人数']+yesterday_clockin_data['昨日线上打卡人数']
    m2_sheet = m2_sheet.merge(yesterday_clockin_data,how='left')

    lastweek_clockin_data = get_clockin_data(last_monday,last_monday + pd.DateOffset(days=6))
    lastweek_clockin_data = lastweek_clockin_data.rename(columns={'线下办公人数': '上周线下打卡人数', '线上办公人数': '上周线上打卡人数'})
    lastweek_clockin_data = lastweek_clockin_data[['小组长','上周线下打卡人数','上周线上打卡人数']]
    lastweek_clockin_data['上周打卡总人数'] = lastweek_clockin_data['上周线下打卡人数']+lastweek_clockin_data['上周线上打卡人数']
    m2_sheet = m2_sheet.merge(lastweek_clockin_data,how='left')

    thisweek_clockin_data = get_clockin_data(this_monday,yesterday)
    thisweek_clockin_data = thisweek_clockin_data.rename(columns={'线下办公人数': '本周线下打卡人数', '线上办公人数': '本周线上打卡人数'})
    thisweek_clockin_data = thisweek_clockin_data[['小组长','本周线下打卡人数','本周线上打卡人数']]
    thisweek_clockin_data['本周打卡总人数'] = thisweek_clockin_data['本周线下打卡人数']+thisweek_clockin_data['本周线上打卡人数']
    m2_sheet = m2_sheet.merge(thisweek_clockin_data,how='left')
